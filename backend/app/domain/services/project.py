...
class ProjectTypeService:
    ...
    async def get_stats(self, id: int) -> dict:
        ...
        projects = await self.project_repo.get_all_filtered(
            limit=10000, project_type_ids=[id]
        )
    ...
    async def migrate_projects(
        self, current_type_id: int, target_type_id: int, status_map: dict[str, str]
    ) -> int:
        """
        ...
        for new_status in status_map.values():
            if new_status not in target_workflow:
                raise ValidationError(
                    f"Target status '{new_status}' not in target workflow"
                )

        # Perform Migration
        projects = await self.project_repo.get_all_filtered(
            limit=10000, project_type_ids=[current_type_id]
        )
        ...
        for project in projects:
            ...
            await self.project_repo.update(
                project, project_type_id=target_type_id, status=new_status
            )
            migrated_count += 1
    ...
    async def transition_status(
        self, project_type_id: int, old_status: str, new_status: str
    ) -> int:
        """
        ...
        projects = await self.project_repo.get_all_filtered(
            limit=10000, project_type_ids=[project_type_id], statuses=[old_status]
        )
...
class ProjectService:
    ...
    async def list_projects(
        ...
        skip: int = 0,
        limit: int = 100,
        project_type_ids: list[int] = None,
        theme_id: int = None,
    ...
    async def update_project(
        ...
        if project_type_id is not None and project_type_id != project.project_type_id:
            new_project_type = await self.project_type_repo.get_with_fields(
                project_type_id
            )
            ...
            updates["status"] = (
                new_project_type.workflow[0] if new_project_type.workflow else "New"
            )
    ...
    async def delete_project(
        self, project_id: int, target_project_id: int | None = None
    ) -> None:
        """
        ...
        if target_project_id is not None:
            if target_project_id == project_id:
                raise ValidationError(
                    "Cannot move tasks to the same project being deleted"
                )
