...
class TeamService:
    ...
    async def delete_team(
        ...
        if task_count > 0:
            if delete_tasks:
                ...
                if not target_team_id:
                    ...
                    if unassigned_team:
                        ...
                        raise ValidationError(
                            "No target team specified and unassigned team not found"
                        )
                ...
                # Reassign tasks
                await self._reassign_team_tasks(
                    team_id,
                    target_team_id,
                    target_task_type.id if target_task_type else None,
                )
    ...
    async def _reassign_team_tasks(
        self, source_team_id: int, target_team_id: int, target_task_type_id: int | None
    ) -> None:
        ...
        for task in tasks:
            ...
            if target_task_type_id:
                ...
                target_type_query = select(TaskType).where(
                    TaskType.id == target_task_type_id
                )
